// ============================================================
// MONGO_GARDEN - Migracion MKR1000 -> ESP32-S3
// Version: 2026-08-23
//
// CAMBIOS DE MIGRACION:
//  1. WiFi101.h -> WiFi.h (WiFi nativo del ESP32)
//  2. Se elimina USE_WIFI101_PRO (WebSocketsServer_Generic detecta ESP32 solo)
//  3. WiFi.getTime() no existe en ESP32 -> NTP con configTime()/getLocalTime()
//  4. CRITICO: en ESP32, SD.open(path, FILE_WRITE) TRUNCA el archivo (modo "w").
//     El respaldo CSV ahora abre con FILE_APPEND.
//  5. ADC: solo pines ADC1 (GPIO1-10). ADC2 (GPIO11-20) NO funciona con WiFi activo.
//     analogReadResolution(10) mantiene la escala 0-1023 del MKR1000.
//  6. analogWrite(pin, 1023) de los reles -> digitalWrite (duty 100% = HIGH)
//  7. WiFi.mode(WIFI_STA) + manejo de red abierta (password vacio)
//  8. Pines remapeados (ver bloque de pines mas abajo)
// ============================================================
...
//libsAndUtils
  ...
  #include <SD.h>
  #include <SPI.h>
  #include <WiFi.h>                      // <- antes WiFi101.h
  #include <MQTT.h>
...
// Pines evitados a proposito:
//   GPIO0, 3, 45, 46  -> strapping (boot/JTAG/VDD_SPI)
//   GPIO19, 20        -> USB nativo (D-/D+)
//   GPIO26-32         -> flash SPI interna
//   GPIO33-37         -> PSRAM octal (modulos -R8)
//   GPIO43, 44        -> UART0 (debug)
// ADC: SOLO ADC1 (GPIO1-10). ADC2 queda bloqueado mientras WiFi este activo.
// ============================================================
...
//startVars-
  ...
  ThreeWire wireClock(RTC_IO, RTC_CLK, RTC_CE);   // (IO, SCLK, CE)
  RtcDS1302<ThreeWire> Rtcmod(wireClock);
  String dateTime;


  DHT dhtexterno(DHT_EXT_PIN, DHT11);
    float internalTemp;
    float internalAirHumidity;


  DHT dhtinterno(DHT_INT_PIN, DHT11);
...
void setup()
{
  ...
  pinMode(DHT_EXT_PIN, INPUT);
  pinMode(DHT_INT_PIN, INPUT);
  pinMode(CS_PIN, OUTPUT); // CS PIN
  //--------------ESTO NO HACE FALTA DECLARARLO, PERO SE DEJA COMENTADO PARA TERMINOS PRACTICOS------
  // pinMode(SD_MOSI, OUTPUT); // MOSI PIN  (GPIO11)
  // pinMode(SD_SCK, OUTPUT);  // SCK PIN   (GPIO12)
  // pinMode(SD_MISO, INPUT);  // MISO PIN  (GPIO13)
  // I2C: SDA=GPIO8, SCL=GPIO9 (Wire.begin los configura)
  // RTC DS1302: IO=GPIO41, SCLK=GPIO42, CE=GPIO7 (ThreeWire por software, no requiere pinMode)
  //-------------------------------------------------------------------------------------------------

  // ADC: misma escala 0-1023 que el MKR1000 y rango completo ~0-3.3 V
  ...
  Wire.begin(I2C_SDA, I2C_SCL);          // BMP180 + TSL2561
  SPI.begin(SD_SCK, SD_MISO, SD_MOSI);   // bus SPI del modulo SD

  WiFi.mode(WIFI_STA);                   // ESP32: hay que fijar el modo antes de begin()
  WiFi.setSleep(false);                  // evita latencias por ahorro de energia

  Serial.begin(115200);   // Si usas el USB nativo del S3: habilita "USB CDC On Boot" en el IDE
  delay(5000);
  Serial.println("..--..--..--..");
...
void loop()
{
  ...
  // Respaldo en SD mientras MQTT este caido.
  // NOTA MIGRACION: la funcion existia en el sketch original; si en tu
  // version final no la llamabas desde loop(), borra esta linea.
...
String syncRtcWithInternet()
{
    ...
    // MIGRACION: el ESP32 no tiene WiFi.getTime().
    // Se usa el cliente NTP del core: configTime() + getLocalTime().
    configTime(utcOffsetSeconds, 0, "pool.ntp.org", "time.nist.gov", "time.google.com");

    struct tm timeinfo;
    if (!getLocalTime(&timeinfo, 10000)) {   // espera hasta 10 s la respuesta NTP
...
bool connectWiFi()
{
  ...
  for (uint8_t networkIndex = 0; networkIndex < WIFI_NETWORK_COUNT; networkIndex++) {
    ...
    for (uint8_t attempt = 0; attempt < maxRetriesPerNetwork; attempt++) {
      ...
      WiFi.disconnect();   // limpia intentos previos (importante en ESP32)

      // MIGRACION: red abierta (password vacio) vs red con password.
      // En ESP32, WiFi.begin(ssid, "") falla; hay que llamar WiFi.begin(ssid).
      if (strlen(wifiPasswords[networkIndex]) == 0) {
        WiFi.begin(wifiSsids[networkIndex]);
      } else {
...
void connectMQTT()
{
  ...
  while (!mqttClient.connect(
           "mongo_garden_esp32s3",   // <- antes "mongo_garden_mkr1000"
           MQTT_USERNAME,
           MQTT_PASSWORD
...
String openRightValve(unsigned long time)
{
  ...
  // MIGRACION: analogWrite(RELAYR, 1023) en MKR1000 = duty 100% = digitalWrite HIGH
...
String openLeftValve(unsigned long time)
{
  ...
  // MIGRACION: analogWrite(RELAYL, 1023) en MKR1000 = duty 100% = digitalWrite HIGH
...
bool logOfflineSnapshot()
{
  ...
  // MIGRACION CRITICA: en ESP32, FILE_WRITE abre en modo "w" y TRUNCA
  // el archivo. Para agregar filas al CSV hay que usar FILE_APPEND.
  File file = SD.open(SENSOR_CSV_PATH, FILE_APPEND);
